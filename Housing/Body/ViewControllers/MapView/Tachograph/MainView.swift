...
class MainView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        if bounds.size.width > bounds.size.height {
            ...
            rectButton.frame = CGRect(x: bounds.size.width - rectButton.frame.size.width - margin, y: bounds.size.height - rectButton.frame.size.height - margin - tabbarH - 64, width: rectButton.bounds.size.width, height: rectButton.bounds.size.height)
        }
...
//        MSGLog(Message: "\(appconfig.showSmallMap)")
    ...
    private func addGestureForMapView() {
        let panGesture = UIPanGestureRecognizer(target: self, action: #selector(panGesture(_:)))
        rectButton.addGestureRecognizer(panGesture)
    }
